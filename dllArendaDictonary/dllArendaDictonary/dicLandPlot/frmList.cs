...
namespace dllArendaDictonary.dicLandPlot
{
    public partial class frmList : Form
    {
        ...
        {
            if (DialogResult.OK == new frmAdd() { Text = "Добавить земельный участок" }.ShowDialog())
                get_data();
        }
        ...
        private void btEdit_Click(object sender, EventArgs e)
        {
            if (dgvData.CurrentRow != null && dgvData.CurrentRow.Index != -1 && dtData != null && dtData.DefaultView.Count != 0)
            {
                ...
                if (DialogResult.OK == new frmAdd() { Text = "Редактировать земельный участок", row = row }.ShowDialog())
                    get_data();
            }
        }
        ...
        private void setFilter()
        {
            ...
            try
            {
                ...
                if (tbNumber.Text.Trim().Length != 0)
                    filter += (filter.Length == 0 ? "" : " and ") + $"NumberPlot like '%{tbNumber.Text.Trim()}%'";
